...
public class WallJumpTraning : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        WallJumpText = transform.Find("TextBox").GetComponent<Transform>();
    ...
    public void F_SetArrow(BoxPoint num)
    {
        ...
        else if(num == BoxPoint.startpoint)
        {
            ...
            WallJumpText.gameObject.SetActive(true) ;
            //GameManager.Instance.player.F_CharText("WallJumpFail");
            //콜라이더밟으면 스페이스바 누르라고 하는 채팅나왓는데 UI로 만들거임 없어도됨
        }
    ...
    public void F_TriggetExit(BoxPoint num)
    {
        ...
        {
            start = false;
            WallJumpText.gameObject.SetActive(false) ;
        }
